...
def model(Y, t, d):
    ...
    y = myinterpolator(t)
    update(myinterpolator, t, original_phage.infection_rate(c_host_b, c_poo) - y - out_b(t)*c_inf_poo)
    if y != 0:
        print(y)
#    print(y)
    ...
    return np.array([0 if c_host_a < 0 else new_host.per_cell_growth_rate(c_nutr_a, temperature_a(t))*c_host_a - out_a(t)*c_host_a - new_host.death_rate*c_host_a,
        ...
        new_phage.infection_rate(c_host_b, c_pnn) if c_inf_pnn < 0 else new_phage.infection_rate(c_host_b, c_pnn) - y - out_b(t)*c_inf_pnn,
        0 if c_poo < 0 else original_phage.burst_size*y - original_phage.infection_rate(c_host_b, c_poo)
        - out_b(t)*c_poo - original_phage.death_rate*c_poo + in_lib(t)*(1 - R_pnn),
        0 if c_pnn < 0 else new_phage.burst_size*y - new_phage.infection_rate(c_host_b, c_pnn)
