...
n=2
reactorA = [new_host.per_cell_growth_rate(y(1,t-1),37)*y(0) - out_a(t)*y(0) -new_host.death_rate*y(0),
            - new_host.yield_coeff*new_host.per_cell_growth_rate(y(1,t-1),37)*y(0) + s0*in_a_lb(t) - y(1)*out_a(t)]
def dX():
    for i in range(n):
        yield reactorA[i]

initial_state = np.array([new_host.c0,s0])
DDE = jitcdde(dX, verbose=True)

DDE.constant_past([new_host.c0,s0])

DDE.step_on_discontinuities()

data = []
x = np.arange(DDE.t, DDE.t+120, 120/100)
for time in x:
