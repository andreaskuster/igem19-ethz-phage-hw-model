...
def c_nutr_b(tt, c_host_b, c_inf_poo, c_inf_pnn, c_nutr_a):
    return - new_host.yield_coeff*new_host.per_cell_growth_rate(c_nutr_b, temperature_b(tt))*(c_host_b+c_inf_poo+c_inf_pnn) + s0*in_b_lb(tt)*0 + c_nutr_a*in_nh(tt)*0 - c_nutr_b*out_b(tt)*0
...
q_h_inf_pnn = deque([0])
for i in range(100):
    q_h_inf_poo.append(0)
    q_h_inf_pnn.append(0)
...
def model(Y, t, d):
    ...
    q_h_inf_poo.append(c_inf_poo)
    q_h_inf_poo.append(c_inf_poo)
    prev = q_h_inf_poo.popleft()
    prev2 = q_h_inf_poo.popleft()
    diff = prev-prev2
    q_h_inf_pnn.append(c_inf_pnn)
    q_h_inf_pnn.append(c_inf_pnn)
    prevn = q_h_inf_pnn.popleft()
    prev2n = q_h_inf_pnn.popleft()
    diffn = prevn-prev2n
    if diffn != 0:
        print(diffn)
    ...
    return np.array([0 if c_host_a < 0 else new_host.per_cell_growth_rate(c_nutr_a, temperature_a(t))*c_host_a - out_a(t)*c_host_a - new_host.death_rate*c_host_a,
        ...
        original_phage.infection_rate(c_host_b, c_poo) if c_inf_poo < 0 else original_phage.infection_rate(c_host_b, c_poo) - diff - out_b(t)*c_inf_poo,
        new_phage.infection_rate(c_host_b, c_pnn) if c_inf_pnn < 0 else new_phage.infection_rate(c_host_b, c_pnn) - diffn - out_b(t)*c_inf_pnn,
        0 if c_poo < 0 else original_phage.burst_size*diff - original_phage.infection_rate(c_host_b, c_poo)
        - out_b(t)*c_poo - original_phage.death_rate*c_poo + in_lib(t)*(1 - R_pnn),
        0 if c_pnn < 0 else new_phage.burst_size*diffn - new_phage.infection_rate(c_host_b, c_pnn)
